 ...
 *
 * @uses next_travel_header_style()
 */
function next_travel_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'next_travel_custom_header_args', array(
		'default-image'          => '%s/assets/uploads/header-image.jpg',
		'default-text-color'     => '181818',
		'width'                  => 1200,
		'height'                 => 528,
		'flex-height'            => true,
		'wp-head-callback'       => 'next_travel_header_style',
	) ) );
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'next-travel' ),
		),
	) );
}
add_action( 'after_setup_theme', 'next_travel_custom_header_setup' );

if ( ! function_exists( 'next_travel_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see next_travel_custom_header_setup().
	 */
	function next_travel_header_style() {
		...
		wp_add_inline_style( 'next-travel-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'next_travel_header_style', 10 );